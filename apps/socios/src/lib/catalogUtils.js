...
export function productIdentityKey(p={}){
 return [p.codigo,p.nombre,p.presentacion].map(normalizeText).join('|');
}
export function toDb(p,{includeId=true}={}){
 p=normalizeProduct(p);
 const row={codigo:p.codigo,nombre:p.nombre,presentacion:p.presentacion,rubro:p.rubro,seccion:p.seccion,precio_lista:p.precioLista,precio_sin_iva:p.precioSinIva,precio_con_iva:p.precioConIva,descuento:p.descuento,precio_sin_iva_descuento:p.precioSinIvaDescuento,precio_con_iva_descuento:p.precioConIvaDescuento,stock:p.stock,activo:p.activo!==false,origen:p.origen,updated_at:new Date().toISOString()};
 if(includeId&&p.id) row.id=p.id;
 return row;
}
export function fromDb(row){return normalizeProduct(row);}

function findHeader(raw,names){const h=(raw[0]||[]).map(normalizeText);return h.findIndex(cell=>names.some(n=>cell===normalizeText(n)));}
export async function parseExcel(file,currentProducts=[]){
 const buffer=await file.arrayBuffer(); const wb=XLSX.read(buffer,{type:'array'}); const sheetName=wb.SheetNames.find(n=>normalizeText(n)==='lista de precios')||wb.SheetNames[0];
 if(!sheetName) throw new Error('El archivo no contiene hojas.');
 const raw=XLSX.utils.sheet_to_json(wb.Sheets[sheetName],{header:1,defval:''}); if(!raw.length) throw new Error('La hoja está vacía.');
 const systemId=findHeader(raw,['ID Sistema','ID interno','System ID']);
 const code=findHeader(raw,['Código','Codigo']); const name=findHeader(raw,['Producto','Nombre','Descripción','Descripcion']); const pres=findHeader(raw,['Presentación','Presentacion']); const rubro=findHeader(raw,['Rubro','Categoría','Categoria']); const section=findHeader(raw,['Sección','Seccion']); const stock=findHeader(raw,['Stock','Existencia','Existencias']); const list=findHeader(raw,['Precio de lista','Precio lista']); const disc=findHeader(raw,['Descuento']); const sin=findHeader(raw,['Precio S/IVA','Precio S IVA','Precio sin IVA']); const con=findHeader(raw,['Precio C/IVA','Precio C IVA','Precio con IVA']);
 if(code<0||name<0||(list<0&&sin<0)) throw new Error('El Excel debe tener Código, Producto y Precio de lista o Precio S/IVA.');

 const byId=new Map(currentProducts.filter(p=>p.id).map(p=>[String(p.id),p]));
 const queues=new Map();
 for(const p of currentProducts){const k=productIdentityKey(p);if(!queues.has(k))queues.set(k,[]);queues.get(k).push(p);}
 const usedIds=new Set();

 return raw.slice(1).map(row=>{
  const c=String(row[code]||'').trim(),n=String(row[name]||'').trim(); if(!c||!n) return null;
  const presentation=pres>=0?String(row[pres]||'').trim():'';
  const explicitId=systemId>=0?String(row[systemId]||'').trim():'';
  let old=explicitId&&byId.get(explicitId)&&!usedIds.has(explicitId)?byId.get(explicitId):null;
  if(!old){
    const key=productIdentityKey({codigo:c,nombre:n,presentacion:presentation});
    const queue=queues.get(key)||[];
    old=queue.find(x=>x.id&&!usedIds.has(x.id))||null;
  }
  if(old?.id) usedIds.add(old.id);
  old=old||{};
  const finalPresentation=pres>=0?row[pres]:old.presentacion;
  ...
  return normalizeProduct({id:old.id||null,codigo:c,nombre:n,presentacion:finalPresentation,rubro:rubro>=0?row[rubro]:old.rubro,seccion:section>=0?row[section]:old.seccion,precioLista:listPrice||sinBase,precioSinIva:sinBase,precioConIva:conBase,descuento:d,precioSinIvaDescuento:d>0?sinBase*(1-d):0,precioConIvaDescuento:d>0?conBase*(1-d):0,stock:stock>=0&&row[stock]!==''?parseNumber(row[stock]):old.stock,activo:true,origen:'IMPORTADO EXCEL'});
 }).filter(Boolean);
}
export function exportExcel(products,filename='PolCarFer - Lista de Precios Actualizada.xlsx'){
 const rows=products.map(p=>({'ID Sistema':p.id||'','Código':p.codigo,'Producto':p.nombre,'Presentación':p.presentacion,'Rubro':p.rubro,'Sección':p.seccion,'Stock':p.stock??'','Precio de lista':p.precioLista,'Descuento %':Math.round(p.descuento*10000)/100,'Precio S/IVA':finalSinIva(p),'Precio C/IVA':finalConIva(p)}));
 const ws=XLSX.utils.json_to_sheet(rows); ws['!cols']=[{wch:38},{wch:16},{wch:58},{wch:20},{wch:26},{wch:34},{wch:12},{wch:16},{wch:14},{wch:16},{wch:16}]; const wb=XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb,ws,'Lista de Precios'); XLSX.writeFile(wb,filename);
}
export function downloadJson(products){const blob=new Blob([JSON.stringify(products,null,2)],{type:'application/json'});const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download='productos.json';a.click();setTimeout(()=>URL.revokeObjectURL(url),500);}
