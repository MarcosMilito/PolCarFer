...
import { AppBar, Box, Button, Container, IconButton, Stack, Toolbar, Typography, useMediaQuery, Menu, MenuItem, Divider } from '@mui/material';
...
export default function AppShell({view,onViewChange,children,cartCount=0}){
 ...
 const choose=id=>{onViewChange(id);setAnchor(null);};
  ...
  <AppBar position="sticky" color="transparent" elevation={0} sx={{backdropFilter:'blur(14px)',borderBottom:'1px solid',borderColor:'divider',backgroundColor:'rgba(11,15,20,.88)'}}>
   <Toolbar sx={{maxWidth:1440,width:'100%',mx:'auto',gap:2}}>
    <Stack direction="row" spacing={1.2} alignItems="center" sx={{mr:'auto',cursor:'pointer'}} onClick={()=>onViewChange('home')}>
     ...
     <Box><Typography fontWeight={900} lineHeight={1}>POLCARFER</Typography><Typography variant="caption" color="text.secondary">Sistema comercial</Typography></Box>
    </Stack>
    {mobile?<>
     <IconButton onClick={e=>setAnchor(e.currentTarget)}><MenuRoundedIcon/></IconButton>
     <Menu anchorEl={anchor} open={Boolean(anchor)} onClose={()=>setAnchor(null)}>
      {nav.map(([id,label])=><MenuItem key={id} selected={view===id} onClick={()=>choose(id)}>{label}{id==='orders'&&cartCount>0?` (${cartCount})`:''}</MenuItem>)}
      <Divider/>
      <MenuItem selected={view==='partner'} onClick={()=>choose('partner')}><LockOutlinedIcon fontSize="small" sx={{mr:1.2}}/>Ingresar al sistema</MenuItem>
     </Menu>
    </>:
    <Stack direction="row" spacing={.5} alignItems="center">
     {nav.map(([id,label])=><Button key={id} color={view===id?'primary':'inherit'} variant={view===id?'contained':'text'} onClick={()=>onViewChange(id)}>{label}{id==='orders'&&cartCount>0?` · ${cartCount}`:''}</Button>)}
     <Button startIcon={<LockOutlinedIcon/>} color={view==='partner'?'primary':'inherit'} variant={view==='partner'?'contained':'outlined'} onClick={()=>onViewChange('partner')} sx={{ml:1}}>Ingresar al sistema</Button>
    </Stack>}
   </Toolbar>
  </AppBar>
